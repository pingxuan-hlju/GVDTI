...
class classfier(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(1, 8, kernel_size=(1, 2), stride=2, padding=1),
            # nn.BatchNorm2d(8),
            nn.PReLU(),
            nn.MaxPool2d(2, 2),  # (1,6)
            nn.Conv2d(8, 16, kernel_size=(1, 2), stride=2, padding=1),
            # nn.BatchNorm2d(16),
            nn.PReLU(),
            nn.MaxPool2d(2, 2),
        )
    ...
    def forward(self, x):
        ...
        out1 = encode.view(encode.size(0), 16 * 1 * 263)
